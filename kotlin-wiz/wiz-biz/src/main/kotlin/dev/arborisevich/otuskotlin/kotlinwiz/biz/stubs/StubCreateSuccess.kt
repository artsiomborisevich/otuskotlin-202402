...
fun ICorChainDsl<QuizContext>.stubCreateSuccess(title: String, corSettings: QuizCoreSettings) = worker {
    this.title = title
    this.description = """
        Success case for creating an question
    """.trimIndent()
    on { stubCase == QuizStubs.SUCCESS && state == QuizState.RUNNING }
    val logger = corSettings.loggerProvider.logger("stubQuestionSuccess")
